...
const Dictionary = require('./lib/services/dictionaryService');
const MAX_WORDS_IN_GRID = 5;
const SLEEP_TIME = 2000; // Queue is checked every 5 seconds if empty
const GRID_ROWS = 5;
...
require('./lib/api/db').then((db) => {
	...
	const dictionaryService = new Dictionary(db);
        ...
        if(contestId) {
            questionService.getRandomQuestions(contestId).then((contestQuestions) => {
                ...
                contestQuestions.map((ques) => {
                    ...
                    dictionaryService.getRandomWords(MAX_WORDS_IN_GRID - ques.answers.length, 'english').then((ans) => {
                        let g = {
                            bonuses: Array.from(ans[0]),
