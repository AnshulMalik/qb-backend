...
module.exports = {
    ...
    submitAnswer(io, socket, params) {
         ...
         */
        let quesId = params.questionId;
        let answer = params.text;
        let contestId = params.contestId;
        let userId = socket.userId;
        if(!quesId || !answer || !contestId || !userId) {
            return socket.emit(constants.emit.error, {message: 'Some parameter is missing'});
        }

        questionService.verifyAnswer(userId, contestId, quesId, answer).then((resp) => {
            console.log("response from question service: ", resp);
        });
    }
};
