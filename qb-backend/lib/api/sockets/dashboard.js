    ...
    joinContest: (io, socket, params) => {
        ...
        userService.getProfile(socket.userId).then((user) => {
            if(user.contests.indexOf(params.contestId) != -1) {
                ...
                userJoinedResponse.data = { userId: socket.userId };
                io.of(params.contestId).emit(constants.emit.userJoined, userJoinedResponse);
            }
            else {
    ...
    getLeaderboard: (io, socket, params) => {
        ...
        */
        ...
        leaderboardService.getLeaderboard(params.contestId).then((leaderboard) => {
            r.status = statusCodes.SUCCESS;
            r.data = leaderboard;
            socket.emit(constants.emit.getLeaderboard, r);
        })
